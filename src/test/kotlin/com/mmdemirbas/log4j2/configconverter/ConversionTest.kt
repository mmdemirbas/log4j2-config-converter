...
object ConversionTest {
    ...
    fun `from properties - prod`(format: Format) {
        assertCanConvert(config("/com/mmdemirbas/log4j2/configconverter/prod.properties",
                                Properties), format)
    }
    ...
    fun `from properties - sample`(format: Format) {
        assertCanConvert(config("/com/mmdemirbas/log4j2/configconverter/sample.properties",
                                Properties), format)
    }
    ...
    fun `from yaml - prod`(format: Format) {
        assertCanConvert(config("/com/mmdemirbas/log4j2/configconverter/prod.yaml",
                                Yaml), format)
    }
    ...
    fun `from yaml - sample`(format: Format) {
        assertCanConvert(config("/com/mmdemirbas/log4j2/configconverter/sample.yaml",
                                Yaml), format)
    }
    ...
    fun `from xml - prod`(format: Format) {
        assertCanConvert(config("/com/mmdemirbas/log4j2/configconverter/prod.xml",
                                Xml), format)
    }
    ...
    fun `from xml - sample`(format: Format) {
        assertCanConvert(config("/com/mmdemirbas/log4j2/configconverter/sample.xml",
                                Xml), format)
    }
    ...
    fun `from json - sample`(format: Format) {
        assertCanConvert(config("/com/mmdemirbas/log4j2/configconverter/sample.json",
                                Json), format)
    }

    @Suppress("unused")
    private fun formatsList() = listOf(Properties,
                                       Yaml,
                                       SnakeYaml,
                                       Json/*, Xml*/) // todo: support xml

    private fun config(resourceName: String, format: Format): Config {
        val formatName = format.javaClass.simpleName
        val inputText =
                log("read $resourceName") {
                    ConversionTest::class.java.getResource(resourceName)
                        .readText()
                }
    ...
    private fun assertCanConvert(inputConfig: Config, format: Format) {
        val formatName = format.javaClass.simpleName
        val outputText =
                log("$formatName.save()") { inputConfig.toStringAs(format) }
        val outputConfig =
                log("$formatName.load()") { format.load(outputText.reader()) }
